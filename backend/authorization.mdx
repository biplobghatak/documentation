...
| --- | --- | --- | --- |
| **Pro** | $99/mo ($990/yr) | up to 10 | no |
| **Agency** | $399/mo ($3,990/yr) | unlimited | no |
| **Custom** | contact sales | tailored | - |

<Warning>
Plan pricing in the code (`lib/plans.ts`) is the **source of truth** - Pro $99/mo,
Agency $399/mo, with a unified credit pool. Per-site capabilities are identical
between Pro and Agency; they differ on workspace count and credits per month.
</Warning>

`PlanLimits` (`lib/plans.ts`) holds every quota - `creditsPerMonth`,
`workspaces`, `trackedPrompts`, `trackedKeywordsMax`, `integrationsMax`, allowed citation
...
its members. The org-level credit pool uses the metric key `credits:<orgId>`.
Per-workspace breakdown counters include `ai_credits:<wsId>`,
`tracked_keywords:<wsId>`, `citation_calls:<wsId>`, and `rank_checks:<wsId>`.
