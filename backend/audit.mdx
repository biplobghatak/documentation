...
flowchart TD
  ...
  subgraph ANALYZE["step: analyze-and-store"]
    ...
    A7["update audits + workspaces (scores, botAccess, cwv)"]
  end
...
- **SEO score** — `seoHealthScore(allIssues, pagesCrawled)`: 100 minus weighted
  penalties scaled by pages crawled (only `category: "seo"` issues count, so CWV and
  cross-page SEO contribute here too).
...
| Table | Holds |
|---|---|
| `audits` | one row per run — status, scores, `pagesCrawled`, `billablePages`, `cwv`, `botAccess` |
