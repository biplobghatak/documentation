...
<Note>
The single source of truth is `lib/db/schema.ts` (≈1,390 lines). Every table, column type, index, and enum on this page is pulled from that file with line citations. When this page and the schema disagree, the schema wins - open the file.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="workspaces - one website each (schema.ts:159)">
    ...
    **Article defaults:** `defaultImagePreset`, `featuredImageWidth/Height` (default 1280×720), `contentFocus`, `publishingPace`, `geoModeDefault`, `articleStyle`, `internalLinksCount`, `globalInstructions`, `contentLanguage`, `targetCountry`, author E-E-A-T fields (`authorName/Bio/Url`), and `autoPublishEnabled` (bool, default false; added in `0062` - when on, finished articles auto-publish to the connected integration). Indexed by `userId` and `orgId`.
  </Accordion>
...
| Table | Purpose | Key columns | Notes |
| --- | --- | --- | --- |
| `audits` (`schema.ts:341`) | One audit run per workspace | `status` (`audit_status`: `queued|running|done|error`), `pagesCrawled`, `billablePages`, `seoScore`, `geoScore`, `cwv`, `botAccess` (jsonb `BotAccess`), `fixedIssueTypes`, `aiSuggestions`, `reportJson` (jsonb - full V3 SEO+GEO report, added `0063`; renders the PDF export) | Indexed by workspace. |
| `audit_issues` (`schema.ts:373`) | Issues found in an audit | `url`, `type`, `severity` (`critical|warning|info`), `category` (`seo|geo`), `message`, `fixText` | Joined via `auditId`; carries `userId` for RLS. |
...
| Table | Purpose | Key columns | Notes |
| --- | --- | --- | --- |
| `agent_conversations` (`schema.ts:1222`) | One chat thread per workspace | `title`, `updatedAt`, `lastActiveTools` (jsonb - sticky follow-up routing) | Indexed `(workspaceId, updatedAt)`. |
| `agent_messages` (`schema.ts:1235`) | One AI-SDK `UIMessage` per row | `conversationId`, `role`, `parts` (jsonb - text/tool-calls/reasoning, stored verbatim), `messageKey` (stable per-response id, added `0064`) | Indexed `(conversationId, createdAt)`. See [AI](/backend/ai). |
| `agent_feedback` (`schema.ts:1258`) | Thumbs up/down on one agent response | `workspaceId`, `conversationId`, `messageKey`, `userId`, `rating` (`like\|dislike`), `messagePreview` (denormalized so the vote survives a message delete) | Unique `(userId, messageKey)`; workspace-scoped RLS. Added in `0064`. See [AI](/backend/ai). |
...
Migrations are plain SQL files in `drizzle/`, numbered `0000` → `0064`, with snapshot/journal metadata in `drizzle/meta/` (`_journal.json` lists every applied migration; one `NNNN_snapshot.json` per step records the schema state Drizzle diffs against).
...
| `0062_workspace_auto_publish` | `workspaces.auto_publish_enabled` toggle (auto-publish finished articles). |
| `0063_audit_report_json` | `audits.report_json` - the full V3 report the PDF export renders. |
| `0064_agent_feedback` | `agent_feedback` table + `agent_messages.message_key` (thumbs up/down on agent replies). |
