...
```ts
// lib/crawler/index.ts:239, 92 (UA)
...
`parsePage` (`lib/crawler/index.ts:294`) loads the HTML with Cheerio and extracts
everything the downstream engines need:

```ts
// lib/crawler/index.ts:294
...
rejects non-http(s), and trims the trailing slash (`index.ts:224`).
...
```ts
// lib/crawler/index.ts:659
await Promise.all(Array.from({ length: concurrency }, () => worker()));
```

| Limit | Value / source |
|---|---|
| Wall-clock per crawl | `CRAWL_MAX_MS = 60_000` (`index.ts:518`) |
...
| Broken-link sweep | up to `min(maxPages*3, 150)` sampled links, HEAD-then-GET via an **unbounded** `Promise.all` (`index.ts:667-674`); skipped when `checkLinks: false` |
| Skipped paths | `SKIP_PATH_RE` — login/account/legal/cart/search/feed/sitemap |

A generic `mapWithLimit<T>` helper (`index.ts:168`) provides bounded parallelism, but
it is used by the onboarding `checkLinkStatuses` path (`index.ts:746`), **not** the
...
(`index.ts:251`). The internal onboarding crawl does not pass a validator because
