...
differs on **workspace count** and **credits per month** (`lib/plans.ts`). The shared
limits:

| Limit | Pro | Agency |
| --- | --- | --- |
| `creditsPerMonth` | `10_000` | `50_000` |
| `workspaces` | `10` | unlimited (`-1`) |
| `trackedPrompts` (per workspace) | `50` | `250` |
| `trackedKeywordsMax` (per workspace) | `500` | `500` |
| `integrationsMax` (per workspace) | `5` | `5` |
| `rankFrequency` | `weekly` (daily available) | `weekly` (daily available) |
| `engines` | AI Overview, Gemini, ChatGPT, Claude, Perplexity | same |

<Note>
`inContentImagesPerPost` (`3`) is **defined but no longer enforced** - the image
pipeline hardcodes a cap of **6** in-content images per article
(`lib/inngest/fns/image-pipeline.ts:89`), ignoring this plan value. See
