...
<Warning>
  ...
  "our internal LLM, always Gemini" (`lib/engines/index.ts:9`) — that is wrong:
...
<Note>
  ...
  volume (citation parse, sentiment, intent). A source comment records that
  defaulting it to Kimi "silently multiplies citation-parse cost"
...
const OPENROUTER_AGENT_MODEL = process.env.OPENROUTER_AGENT_MODEL ?? "moonshotai/kimi-k2.6";
...
|---|---|---|---|
| 1 | **OpenRouter** | `OPENROUTER_API_KEY` | `moonshotai/kimi-k2.6` — UI label "Kimi K2.6" (`OPENROUTER_AGENT_MODEL` to override) |
...
routing) and `getAuditAssistantModel()` (the Ask-Spyro audit sidebar uses
`AUDIT_ASSISTANT_MODEL`, which defaults to **MiniMax M2.7**, with a `gpt-4.1-mini`
fallback). So MiniMax now powers the audit sidebar, **not** the main chat agent —
that default moved to Kimi K2.6.
...
- **Credit gating.** A shared per-workspace AI credit pool (chat + keyword search)
  is checked before opening the stream; over-limit returns `402` (`route.ts:162-176`).
- **Grounding up-front.** The system prompt is augmented with the workspace
  ...
  connected?) — each best-effort (`route.ts:256-277`).
- **Locale-aware.** The route injects the workspace's `targetCountry` and
  `contentLanguage` into `buildSpyroTools` and a `countryInstruction` system block
  (`route.ts:182-187, 207-208`), so keyword tools and `recommendBlogIdeas` return
  market/language-appropriate results.
- **Turn routing.** `routeTurn()` selects the **tool subset** the model may use
  ...
  calls (`route.ts:241`). Bare confirmations inherit the previous turn's active
  tools, and `stripIncompleteToolParts` repairs threads so a half-finished tool
  call can't poison the next request (`route.ts:80-95`).
- **Bounded tool loop.** `stopWhen: stepCountIs(10)` caps chained tool calls so a
  confused model can't loop forever (`route.ts:335`); `maxRetries: 2` retries
  transient provider failures, and reasoning is forced off
  (`providerOptions.openrouter.reasoning.effort = "none"`, `route.ts:347-349`).
...
The agent exposes **~27 tools** across two files. `routeTurn()` only *enables* the
subset relevant to a turn, but the full surface is:

| File | Tools |
|---|---|
| `lib/agent/tools.ts` (7 — research/SERP) | `discoverKeywords`, `findQuestions`, `findGems`, `auditPage`, `findCompetitors`, `getCompetitorKeywords`, `validateNiche` |
| `lib/agent/spyro-tools.ts` (20 — site-grounded + content ops) | `searchMySite`, `getWorkspaceSnapshot`, `getRankSummary`, `findContentGaps`, `findRankableTopics`, `suggestInternalLinks`, `recommendBlogIdeas`, `analyzeSerp`, `auditFunnelCoverage`, `getGscOverview`, `getGscQueries`, `getLowCtrPages`, `getAlmostRanking`, `getContentDecay`, `draftArticle`, `getContentSchedule`, `scheduleBlogIdeas`, `rescheduleBlogIdea`, `editBlogIdea`, `sendIdeaToWriter` |

GSC tools (`getGscOverview`/`getGscQueries`/`getLowCtrPages`/`getAlmostRanking`/
`getContentDecay`) only return data when Search Console is connected; the
content-planner tools (`scheduleBlogIdeas`/`rescheduleBlogIdea`/`editBlogIdea`/
`sendIdeaToWriter`) let the agent drive the [calendar](/backend/content-engine)
and hand articles to the writer.
