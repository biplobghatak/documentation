...
4. **Data — Drizzle + Supabase Postgres + RLS** — the server connection uses Drizzle over `postgres-js` and connects as the service role (`lib/db/index.ts`). Postgres RLS (`drizzle/rls.sql`) scopes user-session access by **org/workspace membership** via the `org_role()` / `has_workspace_access()` security-definer helpers (not simple `user_id = auth.uid()` ownership).
...
2. **Postgres RLS** — `drizzle/rls.sql` (plus `rls_site_chunks.sql`) enables **membership-driven** policies via the `has_workspace_access()` / `org_role()` helpers as defence-in-depth for any access made with the **user's own Supabase session**. The server-side Drizzle connection runs as the **service role and bypasses RLS** (`lib/db/index.ts`), which is why it must only be imported from server code and jobs.
